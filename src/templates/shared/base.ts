...
export function basePackageJson(
  ...
  const pkg: Record<string, unknown> = {
    ...
    version: "1.0.0",
    private: true,
    description: `Scaffolded with node-gen-kit — ${config.framework} backend`,
  ...
  if (config.language === "ts") {
    ...
    pkg.main = config.moduleSystem === "esm" ? "./src/index.js" : "./src/index.js";
  }
  return {
    path: "package.json",
...
export function envFiles(config: ProjectConfig): GeneratedFile[] {
  ...
  const names = [".env.example", ".env.local", ".env.dev", ".env.stg", ".env.prod", ".env.test"];
  return names.map((name) => ({
    path: name,
    content: name === ".env.example" ? body : body.replace("NODE_ENV=development", `NODE_ENV=${envName(name)}`),
  }));
}
...
${config.features.auth !== "none" ? "  JWT_SECRET: { type: String, default: 'dev-secret-change-me' },\n  JWT_EXPIRES_IN: { type: String, default: '7d' },\n" : ""}${config.features.database !== "none" ? "  DATABASE_URL: { type: String, default: 'file:./dev.db' },\n" : ""}${config.features.cache === "redis" ? "  REDIS_URL: { type: String, default: 'redis://localhost:6379' },\n" : ""}  STORAGE_PATH: { type: String, default: './uploads' },
});
...
Enterprise Node.js backend generated by [\`node-gen\`](https://www.npmjs.com/package/node-gen).
